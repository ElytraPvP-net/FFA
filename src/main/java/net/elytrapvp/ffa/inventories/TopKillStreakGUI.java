...
public class TopKillStreakGUI extends CustomGUI {
    ...
    public TopKillStreakGUI(Player player) {
        super(54, "Top Kill Streak");
        fillers();
        ...
        Bukkit.getScheduler().runTaskAsynchronously(FFA.getPlugin(), () -> {
            for(int place : LeaderboardUpdate.killStreaks.keySet()) {
                UUID u = LeaderboardUpdate.killStreaks.get(place).u;
                int num = LeaderboardUpdate.killStreaks.get(place).i;

                OfflinePlayer p = Bukkit.getOfflinePlayer(u);
                ItemStack item = new ItemBuilder(Material.PLAYER_HEAD).setDisplayName("&a" + place + ". " + p.getName() + " - " + num + " Kills").build();
