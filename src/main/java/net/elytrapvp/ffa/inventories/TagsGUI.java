...
public class TagsGUI extends CustomGUI {
    ...
    public TagsGUI(Player player, int page, TagType type) {
        ...
        if(page == 1) {
            ...
            setItem(38, new SkullBuilder("754aaab39764641ff8627932d2fe1a4ccced96537508d4abc6cd5fbbb87913").setDisplayName("&aPage " + (page - 1)).build(), (p,a) -> new TagsGUI(player, page - 1, type).open(player));
        }

        if(type.getTags().size() > (page * 21)) {
            setItem(42, new SkullBuilder("61d0f82a2a4cdd85f79f4d9d9798f9c3a5bccbe9c7f2e27c5fc836651a8f3f45").setDisplayName("&aPage " + (page + 1)).build(), (p,a) -> new TagsGUI(player, page + 1, type).open(player));
        }
        else {
