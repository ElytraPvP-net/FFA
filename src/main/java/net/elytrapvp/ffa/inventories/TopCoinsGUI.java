...
public class TopCoinsGUI extends CustomGUI {
    ...
    public TopCoinsGUI(Player player) {
        super(54, "Top Coins");
        fillers();
        ...
        Bukkit.getScheduler().runTaskAsynchronously(FFA.getPlugin(), () -> {
            for(int place : LeaderboardUpdate.coins.keySet()) {
                UUID u = LeaderboardUpdate.coins.get(place).u;
                int num = LeaderboardUpdate.coins.get(place).i;

                OfflinePlayer p = Bukkit.getOfflinePlayer(u);
                ItemStack item = new ItemBuilder(Material.PLAYER_HEAD).setDisplayName("&a" + place + ". " + p.getName() + " - " + num + " Coins").build();
